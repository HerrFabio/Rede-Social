...
import { users, posts, suggestUsers } from './index.js'
...
function openModal(postId) {
  ...
  const modalUser = modal.querySelector(".modal__user");
  const modalImg = modal.querySelector(".modal__img");
  const modalStack = modal.querySelector(".modal__stack");
  const modalText = modal.querySelector(".modal__text");

  modalTitle.textContent = postTitle;
  modalUser.textContent = `Author: ${userName}`;
  modalImg.src = userImageSrc;
  modalStack.textContent = `Stack: ${userOffice}`;
  modalText.textContent = postContent;
